...
function render(){const arr=filtered();$("#total").textContent=items.length;$("#regular").textContent=items.filter(x=>x.type==="恒常商品").length;$("#limited").textContent=items.filter(x=>x.type==="限定商品").length;$("#empty").style.display=arr.length?"none":"block";$("#tableWrap").style.display=arr.length?"block":"none";$("#grid").innerHTML=arr.map(x=>`<tr><td class="nameCell">${esc(x.name)}</td><td><span class="pill">${esc(x.type)}</span></td><td><span class="pill">${esc(x.version)}</span></td><td>${esc(x.effect||"-")}</td><td class="actionCell"><button class="ghost" type="button" data-action="detail" data-id="${esc(x.id)}">詳細</button><button class="ghost" type="button" data-action="edit" data-id="${esc(x.id)}">編集</button></td></tr>`).join("")}
