...
function formatPrice(value){
  ...
  return normalized.replace(/\d{4,}/g,n=>Number(n).toLocaleString("ja-JP"));
}

function hasImageValue(value){
  return String(value||"").trim().length>0;
}
function imageStatusBadges(item){
  const defs=[
    ["開封前",item.beforeImage],
    ["開封後",item.afterImage],
    ["POP",item.popImage]
  ];
  return `<div class="imageStatusRow">${defs.map(([label,value])=>{
    const ok=hasImageValue(value);
    return `<span class="imageStatus ${ok?"has":"none"}">${esc(label)}:${ok?"有":"無"}</span>`;
  }).join("")}</div>`;
}
...
function matchesImageFilter(item,filter){const hasBefore=Boolean(String(item.beforeImage||"").trim()),hasAfter=Boolean(String(item.afterImage||"").trim()),hasPop=Boolean(String(item.popImage||"").trim());if(filter==="before_missing")return !hasBefore;if(filter==="after_missing")return !hasAfter;if(filter==="pop_missing")return !hasPop;if(filter==="missing_any")return !hasBefore||!hasAfter||!hasPop;if(filter==="all_present")return hasBefore&&hasAfter&&hasPop;return true}
function filtered(){const q=$("#q").value.toLowerCase().trim(),t=$("#type").value,v=$("#version").value,img=$("#imageFilter").value;return items.filter(x=>(t==="all"||x.type===t)&&(v==="all"||x.version===v)&&matchesImageFilter(x,img)&&[x.name,x.price,x.effect,x.description,x.memo].join(" ").toLowerCase().includes(q))}
function render(){const arr=filtered();$("#total").textContent=items.length;$("#regular").textContent=items.filter(x=>x.type==="恒常商品").length;$("#limited").textContent=items.filter(x=>x.type==="限定商品").length;$("#wholesale").textContent=items.filter(x=>x.type==="卸商品").length;$("#empty").style.display=arr.length?"none":"block";$("#tableWrap").style.display=arr.length?"block":"none";$("#grid").innerHTML=arr.map(x=>`<tr><td class="nameCell">${imageStatusBadges(x)}<button class="nameLink" type="button" data-action="detail" data-id="${esc(x.id)}">${esc(x.name)}</button></td><td><span class="pill">${esc(x.type)}</span></td><td><span class="pill">${esc(x.version)}</span></td><td>${esc(formatPrice(x.price))}</td><td>${esc(x.effect||"-")}</td><td class="actionCell"><button class="ghost" type="button" data-action="edit" data-id="${esc(x.id)}">編集</button></td></tr>`).join("")}
...
function lockEnterAndSubmit(formSelector){
  ...
  form.addEventListener("keydown",e=>{
    ...
    const allowTextareaNewline=tag==="TEXTAREA" && e.shiftKey;
    if(allowTextareaNewline)return;
    e.preventDefault();
    e.stopPropagation();
  },true);
}
...
lockEnterAndSubmit("#form");
lockEnterAndSubmit("#eventForm");
lockEnterAndSubmit("#columnForm");
...
["#q","#type","#version","#imageFilter"].forEach(s=>$(s).addEventListener("input",render));
...
$("#eventSaveBtn").onclick=saveEventForm;
$("#columnSaveBtn").onclick=saveColumnForm;
