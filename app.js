...
const sampleItems=[
  {id:"ending",name:"ENDING",type:"限定商品",version:"本リリースv2",effect:"なし",beforeImage:svg("ENDING","BEFORE","#102c66","#080713"),afterImage:svg("ENDING","OPENED","#162f4e","#30102d"),popImage:svg("ENDING","POP","#3b2419","#080713"),description:"夜空を横切る彗星のように、静かに広がるアイスミントの余韻。",memo:"プレビュー用サンプルです。"},
  {id:"eclipse",name:"Eclipse teller",type:"恒常商品",version:"本リリースv1",effect:"なし",beforeImage:svg("ECLIPSE","BEFORE","#1a95c9","#07122b"),afterImage:svg("ECLIPSE","OPENED","#0b5470","#180c39"),popImage:svg("ECLIPSE","POP","#1b3244","#07122b"),description:"皆既日食と星の余韻を閉じ込めた、静かなブルーのパッケージ。",memo:"検索・フィルター・詳細表示の動作確認用。"}
];
const sampleEvents=[
...
function fromFirestore(doc){
  const r=doc.data()||{};
  return{id:doc.id,name:r.name||"",type:r.type||"恒常商品",version:r.version||"ベータ",effect:r.effect||"",beforeImage:r.beforeImage||"",afterImage:r.afterImage||"",popImage:r.popImage||"",description:r.description||"",memo:r.memo||""};
}
function toFirestore(i){
  return{name:i.name,type:i.type,version:i.version,effect:i.effect||"",beforeImage:i.beforeImage||"",afterImage:i.afterImage||"",popImage:i.popImage||"",description:i.description||"",memo:i.memo||"",updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
}
...
function filtered(){const q=$("#q").value.toLowerCase().trim(),t=$("#type").value,v=$("#version").value;return items.filter(x=>(t==="all"||x.type===t)&&(v==="all"||x.version===v)&&[x.name,x.effect,x.description,x.memo].join(" ").toLowerCase().includes(q))}
function render(){const arr=filtered();$("#total").textContent=items.length;$("#regular").textContent=items.filter(x=>x.type==="恒常商品").length;$("#limited").textContent=items.filter(x=>x.type==="限定商品").length;$("#wholesale").textContent=items.filter(x=>x.type==="卸商品").length;$("#empty").style.display=arr.length?"none":"block";$("#tableWrap").style.display=arr.length?"block":"none";$("#grid").innerHTML=arr.map(x=>`<tr><td class="nameCell">${esc(x.name)}</td><td><span class="pill">${esc(x.type)}</span></td><td><span class="pill">${esc(x.version)}</span></td><td>${esc(x.effect||"-")}</td><td class="actionCell"><button class="ghost" type="button" data-action="detail" data-id="${esc(x.id)}">詳細</button><button class="ghost" type="button" data-action="edit" data-id="${esc(x.id)}">編集</button></td></tr>`).join("")}
...
$("#form").onsubmit=async e=>{e.preventDefault();const item={id:$("#id").value||localId(),name:$("#name").value.trim(),type:$("#inType").value,version:$("#inVersion").value,effect:$("#effect").value.trim(),beforeImage:$("#before").value.trim(),afterImage:$("#after").value.trim(),popImage:$("#pop").value.trim(),description:$("#desc").value.trim(),memo:$("#memo").value.trim()};try{await saveItem(item);clearForm();show("archive");showSub("product","productListPane")}catch(error){alert("保存エラー: "+error.message)}};
